...
export async function generateChatStream(messages: any[], systemPrompt?: string, options?: { escalate?: boolean }): Promise<ReadableStream> {
  const llmKeys = process.env.LLM_API_KEY
  const baseUrl = (process.env.LLM_BASE_URL || 'https://api.groq.com/openai/v1').replace(/\/$/, '')
  ...
  if (!llmKeys || llmKeys === 'your-llm-api-key') {
    // Basic fallback for now, real ollama streaming can be added later if needed
    throw new Error('Local Ollama streaming not yet implemented. Please configure LLM_API_KEY.')
  }
